...
function AddSide({ refetch }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await API.post("/side/create", formData); // Updated endpoint
      message.success("Side added successfully!");
      refetch();
      handleCancel(); // Close modal on success
    } catch (error) {
      message.error("Failed to add Side. Try again.");
    } finally {
      setLoading(false);
    }
  };
