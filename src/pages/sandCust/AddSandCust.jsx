...
function AddSandCust({ refetch }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await API.post("/sand-cust/create", formData); // Updated endpoint
      message.success("Sandwich Customize added successfully!");
      refetch();
      handleCancel(); // Close modal on success
    } catch (error) {
      message.error("Failed to add Sandwich Customize. Try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <Button type="primary" onClick={showModal}>
        Add Sandwich Customize
      </Button>
      <Modal
        title="Add Sandwich Customize"
        open={isModalOpen}
