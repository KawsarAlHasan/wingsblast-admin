...
function EditDrink({ drinks, isOpen, onClose, refetch }) {
  const { control, register, handleSubmit, reset } = useForm();
  const [loading, setLoading] = useState(false);

  React.useEffect(() => {
    // Reset form fields when drinks changes
    if (drinks) {
      reset({
        name: drinks.name || "",
        cal: drinks.cal || "",
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await API.put(`/drink/update/${drinks?.id}`, formData); // Updated endpoint
